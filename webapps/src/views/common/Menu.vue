<template>
  <div>
    <el-menu class="el-menu-vertical-demo" background-color="#1c2733" text-color="#fff" active-text-color="#ffd04b" :default-openeds="openeds" :default-active="activeMenu.meta && activeMenu.meta.funcId ? activeMenu.meta.funcId : ''">
      <el-submenu v-for="item in menus" :index="item.funcId.toString()" :key="item.funcId" class="first_menu">
        <template slot="title">
          <i :class="item.icon" style=""></i><span >{{ item.name }}</span>
        </template>
        <router-link v-for="menu in item.children" :key="menu.funcId" :to="menu.path">
          <el-menu-item :class="menu.icon" :index="menu.funcId.toString()">
...
export default {
  ...
  data() {
    return {
      menus: [],
      openeds: ["504681454035407046", "504681454035407042", "504681454035407041", "721227411437387776", "754009866309206016"],
    };
  },
  mounted() {
